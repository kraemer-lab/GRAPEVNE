...
def get_rwi_dataframe_from_csv(rwi_csv_file):
    ...
    rwi["geo_id"] = rwi.apply(
        lambda x: get_point_in_polygon(x["latitude"], x["longitude"], polygons), axis=1
    )
...
def get_bing_tile_z14_pop(pop_file):
    ...
    population["quadkey"] = population.apply(
        lambda x: str(quadkey.from_geo((x["latitude"], x["longitude"]), 14)), axis=1
    )
...
shapefile_rwi.plot(
    ax=ax, column="rwi_weight", marker="o", markersize=1, legend=True, label="RWI score"
)
contextily.add_basemap(
    ax, crs={"init": "epsg:4326"}, source=contextily.providers.OpenStreetMap.Mapnik
)
...
def main(shpfile, rwifile, popfile, gid_id, outfile):
    ...
    def get_rwi_dataframe_from_csv(rwi_csv_file):
        ...
        rwi = pd.read_csv(rwi_csv_file)
        rwi["geo_id"] = rwi.apply(
            lambda x: get_point_in_polygon(x["latitude"], x["longitude"], polygons),
            axis=1,
        )
        rwi = rwi[rwi["geo_id"] != "null"]
        return rwi
    ...
    def get_bing_tile_z14_pop(pop_file):
        ...
        bing_tile_z14_pop = population.groupby("quadkey", as_index=False)[
            "pop_2020"
        ].sum()
        bing_tile_z14_pop["quadkey"] = bing_tile_z14_pop["quadkey"].astype(np.int64)
        return bing_tile_z14_pop
