...
if [[ "$RUNNER_OS" == "Windows" ]]; then
    ...
    powershell.exe -Command 'Start-Process .\Miniforge3-Windows-x86_64.exe -ArgumentList "/S /NoRegistry=1 /D=$env:UserProfile\miniforge" -Wait'
    export CONDA_PATH="${USERPROFILE}"/miniforge/condabin
    echo "done."
elif [[ "$RUNNER_OS" == "Linux" ]]; then
    echo "Downloading Miniforge for Linux..."
    wget -O Miniforge3.sh "https://github.com/conda-forge/miniforge/releases/latest/download/Miniforge3-$(uname)-$(uname -m).sh"
    bash Miniforge3.sh -b
    export CONDA_PATH="${HOME}"/miniforge/bin
    echo "done."
